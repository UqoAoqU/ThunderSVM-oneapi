...
class CSMOSolver {
public:
    void solve(KernelMatrix &k_mat, const SyncArray<int> &y, SyncArray<float_type> &alpha, float_type &rho,
               SyncArray<float_type> &f_val, float_type eps, float_type Cp, float_type Cn, int ws_size, int max_iter);

    virtual ~CSMOSolver() = default;

protected:
    void init_f(const SyncArray<float_type> &alpha, const SyncArray<int> &y, KernelMatrix &k_mat,
