...
void
CSMOSolver::solve(KernelMatrix &k_mat, const SyncArray<int> &y, SyncArray<float_type> &alpha, float_type &rho,
    ...
    t0 = high_resolution_clock::now();
	   LOG(INFO) << "k_mat_size:" << k_mat.get_mem_size();
       LOG(INFO) << "nnz: " << k_mat.nnz(); 
       LOG(INFO) << "k_mat.div:" << k_mat.get_div();
    k_mat.init_devices(ws_size);
    ...
    for (int iter = 0;; ++iter) {
...
#ifdef DEBUG
        //     LOG(INFO) << "Check the alpha";
        //     alpha.log(std::cout);
        //     std::cout << '\n';
        //     freopen("now_host.txt", "w", stdout);
        //     auto data = k_mat_rows.host_data();
        //     int ws = k_mat_rows.size() / 4;
        //    for (int i = 0; i < k_mat_rows.size(); ++i)
        //         printf("%f\n", data[i]);
        //     exit(0);
#endif
        } else {
            working_set_first_half.copy_from(working_set_last_half);
            //working_set_first_half -> host
            //working_set_first_half.to_host();
            //working_set -> host
            ...
            // working_set_last_half -> device without data changes
            k_mat.get_rows(working_set_last_half, k_mat_rows_last_half);
        }
        //local smo
#ifdef DEBUG
        LOG(INFO) << "total memory usage:" << (SyncMem::get_total_memory_size() + k_mat.get_mem_size() ) / 1024.0 / 1024.0 <<"MB\n k_mat:";
        f_val.log(std::cout);
#endif //DEBUG
        ...
        //update f
#ifdef DEBUG 
        // LOG(INFO) << "Check the alpha_diff";
        // alpha_diff.log(std::cout);
        // std::cout << std::endl;
#endif 
...
void CSMOSolver::init_f(const SyncArray<float_type> &alpha, const SyncArray<int> &y, KernelMatrix &k_mat,
